...
# View for displaying user's keychain
class KeyChainView(View):
    ...
    def get_layout(self):
        headers = [
            [sg.Column([[sg.Text("Site")]], key='-SITES-'), sg.Column([[sg.Text("Username")]], key='-USERS-'), sg.Column([[sg.Text("Password")]], key='-PASSWORDS-')]
        ]
        site_adder_layout = [
    ...
    def run_event_loop(self):
        ...
        while True:
            ...
            if event == sg.WIN_CLOSED or event == 'Cancel':
                ...
                self.window.write_event_value('-PASS-INP-', password)
